...
function initWhatsAppButton() {
    ...
    btn.href = 'https://wa.me/2348037443347?text=Hi%20Ogbaji%27s%20Expert%20Solutions%2C%20I%27d%20like%20to%20learn%20more%20about%20your%20services.';
    btn.target = '_blank';
    btn.rel = 'noopener';
...
function initExitIntent() {
    if (localStorage.getItem('exitIntentDismissed')) return;
    var overlay = document.createElement('div');
    overlay.id = 'exit-intent-overlay';
    overlay.className = 'exit-intent-overlay';
    overlay.innerHTML = '<div class="exit-intent-modal" role="dialog" aria-modal="true" aria-labelledby="exit-intent-title"><button class="exit-intent-close" onclick="dismissExitIntent()" aria-label="Close">&times;</button><h3 id="exit-intent-title" class="text-2xl font-display font-bold text-primary mb-4">Before You Go...</h3><p class="text-gray-600 mb-6">Get a <strong>free consultation</strong> on how we can transform your business. Schedule a call with our team today.</p><div class="flex flex-col gap-3"><a href="contact.html" class="bg-secondary hover:bg-yellow-600 text-white px-6 py-3 rounded-lg font-semibold text-center transition">Schedule a Consultation</a><a href="https://wa.me/2348037443347" target="_blank" rel="noopener" class="border-2 border-secondary text-secondary hover:bg-secondary hover:text-white px-6 py-3 rounded-lg font-semibold text-center transition">Chat on WhatsApp</a></div></div>';
    document.body.appendChild(overlay);
    var dismissed = false;
    document.addEventListener('mouseleave', function(e) {
        if (dismissed || e.clientY > 0) return;
        overlay.classList.add('visible');
    });
    document.getElementById('exit-intent-overlay').addEventListener('click', function(e) {
        if (e.target === this) dismissExitIntent();
    });
    document.addEventListener('keydown', function(e) {
        if (e.key === 'Escape') dismissExitIntent();
    });
}
...
function dismissExitIntent() {
    ...
    if (overlay) {
        ...
        localStorage.setItem('exitIntentDismissed', '1');
    }
}
...
function initViewTransitions() {
    ...
    document.addEventListener('click', function(e) {
        ...
        if (!a || a.target === '_blank' || a.host !== location.host || a.href === location.href || a.getAttribute('href') === '#') return;
        e.preventDefault();
